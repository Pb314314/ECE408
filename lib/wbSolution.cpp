...
// compare the output file and expected output file.
...
// compare the expect output and generated output
wbBool wbSolution(wbArg_t arg, void * data, int rows, int columns) {
    ...
    expectedOutputFile = wbArg_getExpectedOutputFile(arg);    // get expected output file path from arg
    outputFile = wbArg_getOutputFile(arg);                    // get output file from arg
    type = wbArg_getType(arg);                                // get data type

    wbAssert(type != NULL);
    wbAssert(expectedOutputFile != NULL);
    wbAssert(outputFile != NULL);
    // whether result is right
    res = wbSolution(expectedOutputFile, outputFile, type, data, rows, columns);

    if (res) {  // the answer is correct
        ss << "{\n";
