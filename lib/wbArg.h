...
struct st_wbArg_t {
    int inputCount;                 // number of input files
    char ** inputFiles;             // pointer to an array of char* input path
    char * outputFile;              // pointer to an array of char* output path
    char * expectedOutput;          // expected output path char*
    char * type;                    // data type. (vector)
};
